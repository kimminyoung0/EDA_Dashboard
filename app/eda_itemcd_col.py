...
#         if selected_itemcd and selected_col:
...
#                 save_path = os.path.join(save_dir, f"{selected_col}_compare.png")
...
if uploaded_files:
    ...
    if not all_itemcds:
        ...
        # ✅ 캐시 키 생성: 데이터셋 이름 + 설정 정보 기반 해시
        def generate_hash_key(file_names: List[str], item_cd: str, col: str, kde_option: bool) -> str:
            key_str = "_".join(sorted(file_names)) + f"_{item_cd}_{col}_{kde_option}"
            return hashlib.md5(key_str.encode()).hexdigest()

        hash_key = generate_hash_key(list(data_dict.keys()), selected_itemcd, selected_col, use_kde_for_constant)
        save_dir = f"reports_itemcd_col/distributions_by_item_compare/{selected_itemcd}"
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f"{selected_col}_compare_{hash_key}.png")

        if os.path.exists(save_path):
            #st.info(f"📁 저장된 그래프를 불러옵니다: `{save_path}`")
